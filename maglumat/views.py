...
def index_dinleyjiler(request):
    if request.user.id==1 or request.user.id==3 or request.user.id==11: return render(request, 'dinleyjiler.html',{'d7':'d7'})
...
def index_ishgarler(request):
    if not request.user.id == 3: return render(request, 'ishgarler.html',{'ishgarler': 'ishgarler'})
    else: return HttpResponse(status=204)
...
def rugsatlar(request):
    if request.user.id==1 or request.user.id==2 or request.user.id == 11 : return render(request,'rugsatlar.html',{'rugsatlar':'rugsatlar'})
    else: return HttpResponse(status=204)

def rugsat_table(request): return render(request,'rugsatlar_table.html',{'rugsatlar':Rugsatlar.objects.all()})

def kurslar(request):
    if request.user.id==1 or request.user.id==3  or request.user.id==11: return render(request,'kurslar.html',{'kurslar':Kurslar.objects.all(),'d8':'d8'})
    else: return HttpResponse(status=204)
...
def ir_gidenler_table(request,date):
    ...
    context={'ir_gidenler':asd,'date':date}
