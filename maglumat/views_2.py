        ...
        if form.is_valid():
            ...
            if len(str(got)) == 13:
                ...
                if q==0:
                    ...
                    jasur=Ishgarler.objects.get(at="Aşyrbaýew Jasurbek Bahramowiç")
                    if not jasur:
                        pass
                    else: 
                        if date.strftime('%Y-%m-%d') not in jasur.gelen_wagty:
                            jasur.gelen_wagty[date.strftime('%Y-%m-%d')]="08:00"
                            jasur.save()
        ...
        if request.method=='POST':
            if san==0: wzp=Wezipeler(Wezipe_at=request.POST['at'], is_bashlayar=request.POST['wagt1'], is_gutaryar=request.POST['wagt2'], obed_bashlayar=request.POST['wagt3'], obed_gutaryar=request.POST['wagt4']); wzp.save(); return HttpResponse(status=204,headers={'HX-Trigger': json.dumps({"movieListChanged": None,"showMessage": f"{wzp} wezipesi döredildi."})})
                ...
                def salam():
                    ...
                    for i in Ishgarler.objects.select_related('wezipe'):
                ...
                def hello():
                    ...
                    for i in Dinleyjiler.objects.select_related('kurs'):
                        if i.barkod_san==sen: sana+=1
                    if sana>0: return hello()
